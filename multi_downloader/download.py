...
def downloader(
    ...
    with tqdm(
        ...
        ncols=100,
        colour="green",
        bar_format="{l_bar}{bar} {total_fmt} MB| Download Speed {rate_fmt} | Estimated Time:  {remaining}",
    ) as pbar:
        ...
        google = Google(
            query, limit, image_dir, adult, timeout, filter, verbose, badsites, name
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        "--adult_filter_off",
        action="store_true",
        help="Whether to turn off the adult filter.",
    )
    parser.add_argument(
        "-F",
        "--force_replace",
    ...
    parser.add_argument(
        ...
        type=int,
        default=60,
        help="The timeout for the image download.",
    )
    parser.add_argument(
        "-f",
        "--filter",
        type=str,
        default="",
        help="The filter to apply to the search results.",
    )
    parser.add_argument(
        "-v", "--verbose", action="store_true", help="Whether to print detailed output."
    )
    parser.add_argument(
        "-b",
        "--bad-sites",
        nargs="*",
        default=[],
        help="List of bad sites to be excluded.",
    )
    parser.add_argument(
        "-n", "--name", type=str, default="Image", help="The name of the images."
    )
